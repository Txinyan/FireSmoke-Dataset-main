...
def test(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        #     )  # predictions
        save_num_batches = 5
        if batch_i < save_num_batches:
            # 保存真值图
            f_gt = Path(save_dir) / ("test_batch%g_gt.jpg" % batch_i)
            plot_images(img, targets, paths, str(f_gt), names)

            # 保存预测图
            f_pred = Path(save_dir) / ("test_batch%g_pred.jpg" % batch_i)
            plot_images(
                img, output_to_target(output, width, height), paths, str(f_pred), names
            )

            # # 额外保存高置信度检测结果
            # for si, pred in enumerate(output):
            #     if pred is not None and len(pred) > 0:
            #         high_conf_mask = pred[:, 4] > 0.5
            #         if high_conf_mask.any():
            #             high_conf_pred = pred[high_conf_mask]
            #             f_high = Path(save_dir) / (
            #                 "high_conf_batch%g_img%d.jpg" % (batch_i, si)
            #             )
            #             plot_images(
            #                 img[si : si + 1],
            #                 output_to_target([high_conf_pred], width, height),
            #                 [paths[si]],
            #                 str(f_high),
            #                 names,
            #             )
