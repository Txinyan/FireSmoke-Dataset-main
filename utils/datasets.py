...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(
        ...
        # 调试：检查 shapes 里面是不是有奇怪的元素
        for i, s in enumerate(shapes):
            try:
                _ = np.array(s, dtype=np.float64)
                # 检查长度是否为 2 [h, w]
                if np.array(s).size != 2:
                    print(
                        "Bad shape length at index:",
                        i,
                        "value:",
                        s,
                        "file:",
                    ...
                    raise ValueError("Bad shape length")
            except Exception as e:
                print("Bad shape at index:", i, "value:", s, "file:", self.img_files[i])
                raise e
    ...
    def __getitem__(self, index):
        ...
        nL = len(
            labels
        )  # number of labels  为空  怎么处理的？？？  但是为啥我添加负样本就报错呢？？
        if nL:
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        r = self.img_size / max(
            h0, w0
        )  # resize image to img_size  在mosaic之前 先将图像最大边resize至640
...
def load_mosaic(self, index):
    ...
    ]  # 3 additional image indices  从图像集中随机选择3张+原始1张；
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            x1a, y1a, x2a, y2a = (
                ...
                xc,
                yc,
            )  # xmin, ymin, xmax, ymax (large image)  填入左上框内图像的 左上 右下 坐标；这是对应mosaic框
            x1b, y1b, x2b, y2b = (
                w - (x2a - x1a),
                h - (y2a - y1a),
                w,
                h,
            )  # xmin, ymin, xmax, ymax (small image)  均大于0，这是裁剪图像的坐标，对应原始图像
        elif i == 1:  # top right
        ...
        padw = (
            x1a - x1b
        )  # mosaic框左上角坐标 - 图像左上角坐标，外面为裁剪 需要减pad，里面为填充，需要加pad
        padh = y1a - y1b
...
def random_perspective(
    ...
    if n:
        ...
        if (
            perspective
        ):  # 完全可以采用cv2  [x,y,1]->[x',y',1] 无论什么变换后，第3个坐标都要变成1，其它2维度根据其放缩
