...
def train(hyp, opt, device, tb_writer=None):
    ...
    freeze_layers = 10  # 冻结前10层（0-9）
    print(f"🚀 Freezing first {freeze_layers} layers for fire/smoke detection...")
    freeze_count = 0
    ...
    for i, (name, param) in enumerate(model.named_parameters()):
        if i < freeze_layers:
            ...
            freeze_count += 1
            if freeze_count <= 5:  # 只打印前5个冻结层
                print(f"  ❄️ Frozen: {name}")
    if freeze_count > 0:
        total_params = sum(p.numel() for p in model.parameters())
        trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"📊 Freeze Summary:")
        print(f"   Frozen layers: {freeze_count}")
        print(f"   Total params: {total_params:,}")
        print(f"   Trainable params: {trainable_params:,}")
        print(
            f"   Frozen ratio: {(total_params - trainable_params) / total_params:.1%}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, default="data/coco128.yaml", help="fire_smoke.yaml path"
    )
    parser.add_argument(
        "--hyp",
        type=str,
    ...
    parser.add_argument(
        "--img_size", nargs="+", type=int, default=[640, 640], help="train,test sizes"
    )
    ...
    parser.add_argument(
        "--cache_images", action="store_true", help="cache images for faster training"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of layers to freeze (starting from 0)",
    )
    ...
    if opt.resume:
        last = (
            get_latest_run() if opt.resume == "get_last" else opt.resume
    ...
    if not opt.evolve:
        ...
        if opt.global_rank in [-1, 0]:
            ...
            tb_writer = SummaryWriter(
                log_dir=increment_dir(Path(opt.logdir) / "exp_Re", opt.name)
            )  # runs/exp
